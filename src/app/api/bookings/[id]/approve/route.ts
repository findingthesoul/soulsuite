// Public token-gated approval endpoint. The host receives an email with an Approve URL
// containing the per-booking approvalToken; clicking it confirms the booking and runs the
// standard finalize path (Google event + invitee confirmation email).
//
// We support both GET (email click) and POST (programmatic). Idempotent on already-CONFIRMED.
...
async function handle(request: NextRequest, id: string): Promise<NextResponse> {
  ...
  // GET clicks land on a friendly confirmation page; POSTs get JSON.
  if (request.method === "GET") {
    const slug = booking.project?.slug ?? booking.host.slug;
    const url = `${publicEnv.NEXT_PUBLIC_APP_URL}/${slug}/${booking.meetingType.slug}/confirmed/${id}?approved=1`;
