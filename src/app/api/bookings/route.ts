...
export async function POST(request: NextRequest) {
  ...
  // Adyen was scaffolded in 1.3.0 but Adyen declined to onboard the platform; reject the
  // legacy enum value with a clear message instead of routing through dead code.
  if (isPaid && meetingType.paymentMethod === "ADYEN") {
    return new NextResponse(
      "Adyen is no longer supported. Switch this meeting type to Stripe or Invoice.",
      { status: 503 },
    );
  }
