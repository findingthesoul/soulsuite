...
function validatePricing(draft: DraftValues, hostHasStripe: boolean): string | null {
  ...
  if (draft.paymentMethod === "ADYEN") {
    return "Adyen is no longer supported. Pick Stripe or Invoice.";
  }
  const major = Number(draft.priceMajor);
  if (!Number.isFinite(major) || major <= 0) {
...
function validatePersonalDraft(
  ...
  if (draft.conferencingProvider === "PERSONAL_ZOOM_ROOM" && !hostHasPersonalZoomRoom) {
    errors.push({
      tabKey: "conferencing",
      message: "Set your Zoom room URL on your profile before using it here.",
    });
  }
...
function EditMeetingTypeForm({
  ...
  function save() {
    setError(null);
    const errors = validatePersonalDraft(
      draft,
      hostHasZoom,
      hostHasStripe,
      hostHasPersonalZoomRoom,
